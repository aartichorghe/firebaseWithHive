...
class UserRepository {
  ...
  // Fetch users from Firebase and sync with Hive
  Stream<List<UserModel>> getUsersFromFirebase() {
    return userCollection.snapshots().map((snapshot) {
      List<UserModel> firebaseUsers = snapshot.docs.map((doc) => doc.data()).toList();

      // Sync Firebase data with Hive
      firebaseUsers.forEach((user) async {
        var existsInHive = _userBox.values.any((existingUser) => existingUser.id == user.id);
        if (!existsInHive) {
          ...
          if (index != -1) {
            await _userBox.putAt(index, user);  // Update user in Hive
          }
        }
      });
  ...
  // Fetch users from Hive
  List<UserModel> getUsersFromHive() {
    if (!Hive.isBoxOpen('userBox')) {
      initHive();
    }
    print('getUsersFromHive ${_userBox.values.toList()}');
  ...
  Future<void> updateUser(String userId, UserModel user) async {
    await userCollection.doc(userId).update(user.toJson());  // Update Firebase
  ...
  Future<void> fetchDataAndStoreInHive() async {
    ...
    if (!await hasLocalData()) {
      ...
      for (var doc in firebaseUsers.docs) {
        ...
        bool existsInHive = _userBox.values.any((existingUser) => existingUser.id == user.id);
        if (!existsInHive) {
          await _userBox.add(user);
        }
      }
      print('Users fetched from Firebase and stored in Hive ${_userBox.values.toList()}');
    }
  }
}
*/
    ...
    return userCollection.snapshots().map((snapshot) {
      ...
      firebaseUsers.forEach((user) async {
        ...
        if (!existsInHive) {
          ...
          var index = _userBox.values.toList().indexWhere((u) => u.id == user.id);
          if (index != -1) {
            await _userBox.putAt(index, user);  // Update user in Hive if data changes
          }
        }
      });
